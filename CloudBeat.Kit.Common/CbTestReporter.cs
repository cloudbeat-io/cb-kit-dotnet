...
namespace CloudBeat.Kit.Common
{
	public class CbTestReporter
	{
		...
		public void StartCase(string name, string fqn, Action<CaseResult> updateAction = null)
		{
			...
			var newCase = parentSuite.AddNewCase(name, fqn);
			if (updateAction != null)
		...
		{
			StepResult newStep = StartStep(caseFqn, stepName);
			if (newStep == null)
				return func.Invoke();
		...
		public TResult Step<T, TResult>(CaseResult parentCase, string stepName, StepTypeEnum stepType, Func<T, TResult> func, T arg, Action<StepResult> updateStepAction = null)
        {
			...
			StepResult newStep = StartStep(parentCase, stepName);
			if (newStep == null)
				return func.Invoke(arg);
		...
		public StepResult Step(string caseFqn, string stepName, Action action)
		{
			...
			{				
				action();
				EndStep(caseFqn, newStep);
			}
			catch (Exception e)
			{
				var failure = CbExceptionHelper.GetFailureFromException(e);
				EndStep(caseFqn, newStep, TestStatusEnum.Failed);
				newStep.Failure = failure;
				throw;
			}
			return newStep;
		...
		public virtual StepResult StartStep(string stepName)
		{
			...
			if (parentCase == null)
				return null;
			return parentCase.AddNewStep(stepName);
		}
		public StepResult StartStep(string caseFqn, string stepName)
        {
			var parentSuite = _result?.GetSuiteByCaseFqn(caseFqn);
			var parentCase = parentSuite?.GetCaseByFqn(caseFqn);
			if (parentCase == null)
			...
			StepResult newStep = parentCase.AddNewStep(stepName);
			return newStep;
		}

		public StepResult StartStep(CaseResult parentCase, string stepName)
		{
			if (parentCase == null)
				return null;
		...
		public StepResult StartStep(string suiteFqn, string caseFqn, string stepName)
		{
			var parentSuite = _result?.GetSuite(suiteFqn);
			var parentCase = parentSuite?.GetCaseByFqn(caseFqn);
			if (parentCase == null)
		...
		public virtual StepResult EndStep(StepResult step, TestStatusEnum? status = null)
        {
			throw new NotImplementedException();
        }

		public StepResult EndStep(StepResult step, CaseResult parentCase, TestStatusEnum? status = null)
		{
